...
import eMarket.controller.ItemFormDto;
...
public class ProductStepDefsTest {
    ...
    public void the_order_X_contains_an_item_with_products_and_the_total_cost_of_the_order_is(int arg1, String productname, String cost) throws Throwable {
    	store = getStore();
    	//assertThat("the order X contains",store.getProductList().size()==arg1);
    }
    ...
    public void i_do_a_get_for_the_order_X_and_item_containing(String request, String arg2) throws Throwable {
    	result = this.mockMvc.perform(get(request));
    }

    @Then("^the order X does not contain the item with \"([^\"]*)\" and the total cost of the order is \"([^\"]*)\"$")
    public void the_order_X_does_not_contain_the_item_with_and_the_total_cost_of_the_order_is(String arg1, String arg2) throws Throwable {
    	store = getStore();
    }
